...
export class BooksdataComponent implements OnInit {
  ...
  filterBooks() {
    const filteredBooks = this.booksArray.filter(
      (book) =>
  ...
  getAllBooks() {
    this.books.getBooks().subscribe({
      next: (response: any) => {
        // Use response.data if response has a "data" field that holds the books array
        const books = Array.isArray(response) ? response : response.data;

        this.booksArray = books.reverse().map((book: any, index: number) => ({
          ...book,
